...
(function() {
  ...
  // =========================
  // UX MVP: Theme + Toast UI
  // =========================
  (function initThemeAndToast() {

    // Toast helper: lightweight, no layout shifts
    window.showToast = function showToast(title, message, opts = {}) {
      try {
        ...
        // Never break the game if toasts fail
  ...
  // Android hardware back navigation stack.
  // We track main-screen transitions (home/setup/game screens) so the Android
  // back button can pop screens until the home screen is reached.
  let screenBackStack = [];
  let currentMainScreenEl = homeScreen || null;
  let isRestoringAndroidBack = false;
  ...
  function showScreen(screenToShow) {
      ...
      // Record previous screen for Android back.
      // Don't record when we're already restoring via back.
      if (!isRestoringAndroidBack && currentMainScreenEl && screenToShow !== currentMainScreenEl) {
          screenBackStack.push(currentMainScreenEl);
      }
      currentMainScreenEl = screenToShow;
  ...
  function aiMakeMove() {
      ...
      if (linesToDraw > 0) {
          ...
          // Failsafe: if for some reason no lines are detected as available,
          // fall back to drawing the first geometrically valid line anywhere.
          if (availableLines.length === 0) {
              console.warn('AI: No availableLines found, using fallback scan for a valid line');
              for (let r = 0; r <= GRID_SIZE; r++) {
                  for (let c = 0; c <= GRID_SIZE; c++) {
                      // Horizontal candidate
                      if (c < GRID_SIZE) {
                          const hLine = { start: { row: r, col: c }, end: { row: r, col: c + 1 } };
                          if (isValidLine(hLine) && !isLineAlreadyDrawn(hLine)) {
                              availableLines.push(hLine);
                              break;
                          }
                      }
                      // Vertical candidate
                      if (r < GRID_SIZE) {
                          const vLine = { start: { row: r, col: c }, end: { row: r + 1, col: c } };
                          if (isValidLine(vLine) && !isLineAlreadyDrawn(vLine)) {
                              availableLines.push(vLine);
                              break;
                          }
                      }
                  }
              ...
              console.warn('AI: Fallback availableLines count:', availableLines.length);
          }
  ...
  // Called from native Android (MainActivity.onBackPressed()).
  // Must return:
  // - `true`  => web handled back navigation (showed a previous screen)
  // - `false` => native should close the app (we're on the home screen)
  window.__androidBackPressed = function() {
      try {
          // Close overlays first.
          if (
              messageBox &&
              typeof hideMessageBox === 'function' &&
              (() => {
                  try { return window.getComputedStyle(messageBox).display !== 'none'; } catch (e) { return messageBox.style && messageBox.style.display && messageBox.style.display !== 'none'; }
              })()
          ) {
              hideMessageBox();
              return true;
          ...
          if (
              confirmationBox &&
              (() => {
                  try { return window.getComputedStyle(confirmationBox).display !== 'none'; } catch (e) { return confirmationBox.style && confirmationBox.style.display && confirmationBox.style.display !== 'none'; }
              })()
          ) {
              confirmationBox.style.display = 'none';
              return true;
          ...
          // Close game/home modals first (so back doesn't exit the app).
          if (typeof rulesModal !== 'undefined' && rulesModal) {
              rulesModal.style.display = 'none';
              try { document.body.style.overflow = 'auto'; } catch (e) {}
              return true;
          }

          if (typeof infoModal !== 'undefined' && infoModal) {
              infoModal.style.display = 'none';
              try { document.body.style.overflow = 'auto'; } catch (e) {}
              return true;
          }

          // Auth/profile/avatar modals are defined in index.html script.
          try {
              const authModal = document.getElementById('auth-modal');
              if (authModal && window.closeAuthModal && window.getComputedStyle(authModal).display !== 'none') {
          ...
          try {
              ...
              if (userProfile && window.closeUserProfile && window.getComputedStyle(userProfile).display !== 'none') {
          ...
          try {
              ...
              if (avatarSelectionModal && window.closeAvatarSelectionModal && window.getComputedStyle(avatarSelectionModal).display !== 'none') {
                  window.closeAvatarSelectionModal();
                  return true;
              }
          } catch (e) {}
          ...
          try {
              ...
              if (luckyWheelModal && window.getComputedStyle(luckyWheelModal).display !== 'none') {
                  luckyWheelModal.style.display = 'none';
                  try { document.body.style.overflow = 'auto'; } catch (e) {}
                  return true;
              }
          } catch (e) {}

          // online-lobby-ui is nested inside home-screen.
          // When the lobby is open, we should NOT treat it as "home" and exit the app.
          const onlineLobbyUI = document.getElementById('online-lobby-ui');
          let isOnlineLobbyVisible = false;
          if (onlineLobbyUI) {
              try {
                  isOnlineLobbyVisible = window.getComputedStyle(onlineLobbyUI).display !== 'none';
              } catch (e) {
                  // Fallback if getComputedStyle isn't available for some reason
                  isOnlineLobbyVisible = !!(onlineLobbyUI.style && onlineLobbyUI.style.display && onlineLobbyUI.style.display !== 'none');
              }
          }

          if (onlineLobbyUI && isOnlineLobbyVisible) {
              try {
                  if (typeof clearLobbyUIData === 'function') clearLobbyUIData();
              } catch (e) {}

              // Ensure the lobby strip itself is hidden.
              try {
                  onlineLobbyUI.style.display = 'none';
              } catch (e) {}

              isRestoringAndroidBack = true;
              showScreen(homeScreen);
              isRestoringAndroidBack = false;
              return true;
          }

          let isHomeVisibleNow = false;
          if (homeScreen) {
              try {
                  isHomeVisibleNow = window.getComputedStyle(homeScreen).display !== 'none';
              } catch (e) {
                  isHomeVisibleNow = !!(homeScreen.style && (homeScreen.style.display === 'flex' || homeScreen.style.display === 'block'));
              }
          }

          // "True home" means: home-screen is visible, none of the other main screens are
          // visible, and our main-screen back-stack has nothing to go back to.
          let isAnyOtherMainScreenVisible = false;
          const otherScreens = [singlePlayerSetupScreen, twoPlayerSetupScreen, spGameScreen, tpGameScreen, onlineGameScreen];
          try {
              isAnyOtherMainScreenVisible = otherScreens.some((el) => {
                  if (!el) return false;
                  try {
              ...
              // Fallback remains false; we'll be conservative below.
          }

          if (isHomeVisibleNow && !isAnyOtherMainScreenVisible && screenBackStack.length === 0) {
              return false;
          }

          // Otherwise, pop the previous main screen.
          if (screenBackStack.length > 0) {
              const previousScreen = screenBackStack.pop();
              isRestoringAndroidBack = true;
          ...
          // Safety fallback: if stack is empty but we're not on home,
          // show home first (native will close on the next back press).
          if (homeScreen) {
              isRestoringAndroidBack = true;
              showScreen(homeScreen);
              isRestoringAndroidBack = false;
          ...
          console.error('window.__androidBackPressed failed:', e);
          return false;
      }
  };
